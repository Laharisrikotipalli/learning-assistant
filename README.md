...
**Q: The bot isn't responding on Telegram.**

Check logs: `docker logs -f clawtutor-openclaw`. If you see `EFATAL: AggregateError`, it's a DNS issue. Make sure your `docker-compose.yml` has:
```yaml
dns:
  - 8.8.8.8
  - 8.8.4.4
...
**Q: My profile was saved but the bot asks for onboarding again after restart.**

Sessions are in-memory. Profile data is persisted to `/data/memory` via Docker volume, so it survives restarts. If the volume was deleted, re-run onboarding.

**Q: The cron job didn't run at the scheduled time.**

Run `openclaw cron list` to verify the job exists. Check logs at the scheduled time. Test immediately with `openclaw cron trigger "nightly-tech-brief"`.

**Q: How do I reset a user's profile to re-run onboarding?**

```bash
docker exec -it clawtutor-openclaw rm /data/memory/user_YOUR_USER_ID.json
```

Then message the bot — onboarding will restart automatically.

**Q: How do I handle my Telegram token safely?**

Never hardcode it in any file you commit. Define `TELEGRAM_BOT_TOKEN=your-token` in `.env` only. The `.env` file is listed in `.gitignore` and will never be pushed to version control.

**Q: How do I support multiple users with different timezones?**

The onboarding skill captures each user's timezone. For production, implement a post-onboarding hook that creates a per-user cron job using the captured timezone. Alternatively, use a single cron job that iterates all stored profiles and adjusts delivery per timezone.

**Q: The Ollama model is slow. What can I do?**

Switch to `gemma:2b` for speed — update `OLLAMA_MODEL=gemma:2b` in `.env`. Or use a cloud provider by setting `OPENAI_API_KEY` or `ANTHROPIC_API_KEY` and updating `config/openclaw.json`.